...
void adicionarFilme() {
    ...
    // --- Nova Validação: Título e Diretor Juntos ---
    int filmeValido = 0;
    while (!filmeValido) {
        lerString("Titulo: ", novoFilme->titulo, MAX_STR);
        lerString("Diretor: ", novoFilme->diretor, MAX_STR);

        int filmeDuplicado = 0;
        for (int i = 0; i < numFilmes; i++) {
            // Verifica se o título E o diretor são os mesmos (ignora maiúsculas/minúsculas)
            if (strcasecmp(filmes[i]->titulo, novoFilme->titulo) == 0 &&
                strcasecmp(filmes[i]->diretor, novoFilme->diretor) == 0) {
                filmeDuplicado = 1;
                break;
            }
        }
        if (filmeDuplicado) {
            printf("Um filme com este Titulo E Diretor ja existe. Por favor, insira um diferente.\n");
        } else {
            filmeValido = 1;
        }
    }
    ...
    while (!avaliacaoValida) {
        novoFilme->avaliacao = lerFloat("Avaliacao (0-10): ");
